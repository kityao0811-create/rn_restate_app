...
const SettingsItem = ({ icon, title, onPress, textStyle, showArrow = true }: SettingsItemProps) => (
  <TouchableOpacity className='flex flex-row items-center justify-between py-3'
    ...
    <View className='flex flex-row items-center gap-3'>
      ...
      <Text className={`text-lg font-rubik-meidum text-black-300 ${textStyle}`}>{title}</Text>
    </View>
...
const Profile = () => {
  ...
  const handleLogout = async () => {
    ...
    if (result) {
      ...
      Alert.alert("Error", "An error occured white logging out!", [])
    }
  }
      ...
      >
        ...
        <View className='flex-row justify-center flex mt-5'>
          <View className='flex flex-col items-center relative mt-5'>
            <Image
              source={{ uri: typeof user?.avatar === 'string' ? user.avatar : '' }}
